...
- Nav con N pestañas: nunca `grid-cols-N` fijo con más botones que columnas, porque el último salta a la segunda fila. Usar `#tabNav` con `grid-template-columns: repeat(N, ...)` en `>= sm` y desplazamiento lateral por debajo, y una prueba que compare el número de botones con el de columnas.
- Auto-scroll del nav: calcular el desplazamiento con diferencias de `getBoundingClientRect`, nunca con `offsetLeft` (es relativo al ancestro posicionado) ni con `scrollTo({behavior:'smooth'})` (se cancela en este contexto). Asignar `nav.scrollLeft` directamente.
- Gráficas de torta: medir SIEMPRE el texto después de fijar `ctx.font`. Medir antes devuelve el ancho de la fuente por defecto y hace que el detector de colisiones crea que no hay problema.
- Eje de categorías de barras horizontales: tipografía en un único helper (`smallAxisTicks()`), nunca un literal por gráfica — si no, se desincronizan. Para ampliar el margen usar `afterFit` (en `beforeLayout` Chart.js recalcula el ancho después y el cambio no aplica), solo creciendo (`if (target > axis.width)`) para que el área no oscile entre redibujados, y acotando con un máximo del ancho del canvas.
- Abreviar etiquetas largas: dentro de `ticks.callback`, no mutando `tick.label` — Chart.js relee las etiquetas crudas en cada redibujado y sobrescribe lo anterior. El texto completo va en `tooltip.callbacks.title` leyendo de `chart.data.labels`. No usar `crossAlign: 'near'` con `indexAxis: 'y'`: empuja el texto fuera del canvas.
- Decidir si un porcentaje cabe por la geometría real del arco, no por un umbral fijo: límite tangencial `r·tan(φ/2)` y radial `√(R²−r²)`, y sin límite tangencial si la porción supera media vuelta.
- Antes de llamar a un helper nuevo en el runtime, confirmar que existe: un `getAllResponses()` inventado solo explotó en producción, al aplicar un filtro. La prueba `test_every_helper_called_inside_the_runtime_exists` lo cubre.
- Validar el balance de `<div>` del HTML tras reescribir un bloque grande: tres cierres sobrantes sacaron las pestañas 6 y 7 del contenedor y las hicieron parpadear. La prueba `test_html_divs_are_balanced` lo cubre.
- Texto editorial vs. calculado: toda cifra visible debe salir del modelo, nunca de un corte histórico. Las pestañas 6 y 7 son la excepción deliberada: son documentos de decisión y se calculan SIEMPRE sobre el corte completo, sin seguir los filtros.
